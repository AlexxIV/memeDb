const VIEW_ALL_PATH = './views/viewAll.html'
const DETAILS_PATH = './views/details.html'
const ADD_MEME_PATH = './views/addMeme.html'
const REPALCE_ME_STR = '<div id="replaceMe">{{replaceMe}}</div>'
...
function viewAddMeme(req, res) {
  processGetRequest(ADD_MEME_PATH, '', res)
}
...
function getDetails(req, res) {
  ...
  processGetRequest(DETAILS_PATH, currentMemeHtml, res)
}
...
function viewAll(req, res) {
  fs.readFile(__dirname + '/../db/db.json', 'utf-8', (err, data) => {
    ...
    processGetRequest(VIEW_ALL_PATH, replaceContent, res)
  })
}
...
function processGetRequest(pathToFile, replaceContent, res) {
  fs.readFile(pathToFile, (err, data) => {
    ...
    if (replaceContent !== '') {
      data = data.toString().replace(REPALCE_ME_STR, replaceContent)
    }
